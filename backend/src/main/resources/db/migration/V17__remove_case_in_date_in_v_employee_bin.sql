...
SELECT books.books_num                                             AS book_id,
       CASE WHEN (SELECT COUNT(*) > 0
                    FROM reviews
                    WHERE
                    books.books_num = reviews.books_num) = 't'
       THEN
       AVG(reviews.stars)::NUMERIC(2, 1)::TEXT
       ELSE
       'Нет оценок' END                                             AS average_rating
FROM books
...
GROUP BY id, title, author, average_rating, provider, status;
